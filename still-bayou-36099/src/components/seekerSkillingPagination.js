...
function SeekerSkillingPagination() {
    ...
    useEffect(() => {
        const getCards = async () => {
            const res = await fetch(`http://localhost:8000/core/list_seekers_people_all/?page=1&cards=${cardsTotal}`)
    ...
    const fetchCards = async (currentPage) => {
        const res = await fetch(`http://localhost:8000/core/list_seekers_people_all/?page=${currentPage}&cards=${cardsTotal}`)
